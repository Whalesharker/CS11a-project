...
public class King extends Piece{
  ...
  public boolean BlackCheck(int x, int y, int newx, int newy, Piece[][] board){
    ...
    for (int i = 0; i<8; i++){
      if(newx+i<8 && newy+i<8){
        if (board[newx+i][newy+i].getType()=="B" || board[newx+i][newy+i].getType()=="Q"){
          if (board[newx+i][newy+i].getColor()=="W"){
            for (int j = 1; j<i; j++){
      ...
      if (newx-i>0 && newy-i>0){
        if (board[newx-i][newx-i].getType()=="B" || board[newx-i][newx-i].getType()=="Q"){
          if (board[newx-i][newy-i].getColor()=="W")
          for (int j = 1; j<i; j++){
            if(board[newx-j][newy-j].getType()!="e"){
              check = true;
    ...
    //check for king
    if (board[newx+1][newy+1].getType()=="K"){
      check = true;
    }
    if (board[newx+1][newy-1].getType()=="K"){
      check = true;
  ...
  public boolean WhiteCheck(int x, int y, int newx, int newy, Piece[][] board){
    ...
    for (int i = 0; i<8; i++){
      ...
      if (board[i][newy].getType()=="R" || board[i][newx].getType()=="Q"){
        if (board[i][newy].getColor()==)
          for (int j = 1; j < i; j++){
            if ((newy > j && j > i) || (i > j && j > newy)){
              if (board[j][newy].getType() !="e")
              check = true;
