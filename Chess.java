      ...
      while (entered == false){
        ...
        if((y < 0 || y>7 || x < 0 || x>7 || newy < 0 || newx>7 || newx < 0 || newy>7) == false){
          ...
          if  (board[y][x].getColor()== Player){
            ...
            System.out.printf("%n%s%s at %d,%d selected to go to %d,%d which is occupied by %s%s.%n",board[y][x].getColor(),board[y][x].getType(),y,x,newy,newx,board[newy][newx].getColor(),board[newy][newx].getType());
          }
        }
        if(entered == false){
          if(board[y][x].getColor()!= Player){
